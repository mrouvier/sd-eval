...
SdResult::SdResult(const DiaFormat* ref, const DiaFormat* hyp,  int delta, const UEM * uem) {
    fa = miss = confus_ref = confus_hyp = totaltime = ser = conf = correct_time = 0;
...
void sd_evaluate(const DiaFormat* ref, const DiaFormat*  hyp, map<string,map<int, SdResult *> > & result, int delta, const UEM * uem, bool cross) {
    if(!cross) {
